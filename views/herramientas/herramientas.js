...
function save_user_and_update(e) {
 ...
 /* if (!form_validations()) {
  ...
  $.ajax({
    url: "../../controllers/HerramientaController.php?endpoint=save_and_update_herramientas",
    type: "POST",
    data: formData,
    contentType: false,
    processData: false,
...
$(document).ready(function () {  
  ...
  $("#table_herramientas").DataTable({
    ...
    ajax: {
      url: "../../controllers/HerramientaController.php?endpoint=list_herramientas",
      type: "post",
      data: { true: true },
    },
    bDestroy: true,
...
function editar(Id) {
  $.post(
    ...
    function (data) {
      ...
      $("#estado_herramienta").val(data.estado_herramienta);
      $("#nota").val(data.nota); 
    }
  );
  $("#label_title").html("Actualizar Herramienta");
  $("#labelbtn").html("Actualizar");
  $("#modal_mantenimiento").modal("show");
}

function eliminar(Id) {
  //Mostrar un mensaje de confirmación al Herramienta utilizando SweetAlert
  swal
    .fire({
    ...
    .then((result) => {
      if (result.value) {
        ...
        $.post(
          "../../controllers/HerramientaController.php?endpoint=delete",
          { herramienta_id: Id },
          function (data) {
        ...
        $("#table_herramientas").DataTable().ajax.reload();
        //Mostrar un mensaje de éxito al Herramienta utilizando SweetAlert
        swal.fire({
...
$(document).on("click", "#btn_nuevo_registro", function () {
  ...
  $("#label_title").html("Registrar Nuevo Herramienta");
