...
function save_user_and_update(e) {
 ...
 /* if (!form_validations()) {
  ...
  $.ajax({
    url: "../../controllers/CategoryController.php?endpoint=save_and_update_categorias",
    type: "POST",
    data: formData,
    contentType: false,
    processData: false,
...
$(document).ready(function () {  
  ...
  $("#table_category").DataTable({
    ...
    ajax: {
      url: "../../controllers/CategoryController.php?endpoint=list_categorias",
      type: "post",
      data: { true: true },
    },
    bDestroy: true,
...
function editar(Id) {
  $.post(
    "../../controllers/CategoryController.php?endpoint=list_categoria_id",
    { categoria_id: Id },
    function (data) {
      //console.log(data);
  ...
  $("#label_title").html("Actualizar Categoría");
  $("#labelbtn").html("Actualizar");
  $("#modal_mantenimiento").modal("show");
}

function eliminar(Id) {
  //Mostrar un mensaje de confirmación al Categoría utilizando SweetAlert
  swal
    .fire({
    ...
    .then((result) => {
      if (result.value) {
        ...
        $.post(
          "../../controllers/CategoryController.php?endpoint=delete",
          { categoria_id: Id },
          function (data) {
        ...
        $("#table_category").DataTable().ajax.reload();
        //Mostrar un mensaje de éxito al Categoría utilizando SweetAlert
        swal.fire({
...
$(document).on("click", "#btn_nuevo_registro", function () {
  ...
  $("#label_title").html("Registrar Nueva Categoría");
